  ...
  return (
    <main className="h-screen grid grid-cols-2 justify-center items-center">
      <div className="bg-[#fef4ce] h-screen flex justify-center items-center">
        <Image className="" src={`/assets/icon-1.png`} width={400} height={100} alt="Picture for login page" />
      </div>

      <div className="bg-[#e75c45] h-screen flex flex-col justify-between items-center">
        <h2 className="self-end p-3 font-bold text-2xl text-[#fef4ce]">TempatNugas</h2>
        ...
        <div className="w-[400px] bg-[#fef4ce] p-5 rounded-lg [box-shadow:5px_5px_#2e2d33] border-2 border-[#2e2d33] space-y-3 text-[#2e2d33]">
          ...
          <form action={formAction} className="space-y-3">
            ...
            <Button disabled={pending} variant="secondary" size="sm" className="w-full text-center overflow-hidden font-medium">
              Login
            </Button>
            {state?.status === "error" && <p className="text-rose-400">{state?.message}</p>}
          </form>
